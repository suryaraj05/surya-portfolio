    ...
    <div className="pt-[clamp(2rem,5vw,3rem)] pb-[clamp(1.5rem,4vw,2.5rem)]">
      <Container>
        <div className="grid gap-6 md:grid-cols-2">
          <div>
            {previous ? (
              <Link href={`/case-studies/${previous.slug}`} className="block rounded-xl border border-border bg-background p-6 transition hover:opacity-95">
          ...
          <div>
            {next ? (
              <Link href={`/case-studies/${next.slug}`} className="block rounded-xl border border-border bg-background p-6 transition hover:opacity-95 md:text-right">
